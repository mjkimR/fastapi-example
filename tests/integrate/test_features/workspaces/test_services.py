...
class TestWorkspaceServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_create_workspace(self, session: AsyncSession, service: WorkspaceService, regular_user):
        """Should create a new workspace through the service."""
        workspace_data = WorkspaceCreate(name="Service Test Workspace")
        context = {"user_id": regular_user.id}
    ...
    @pytest.mark.asyncio
    async def test_get_workspace(self, session: AsyncSession, service: WorkspaceService, single_workspace: Workspace, regular_user):
    ...
    @pytest.mark.asyncio
    async def test_get_multi_workspaces(self, session: AsyncSession, service: WorkspaceService, sample_workspaces: list[Workspace], regular_user):
    ...
    @pytest.mark.asyncio
    async def test_update_workspace(self, session: AsyncSession, service: WorkspaceService, single_workspace: Workspace, admin_user):
    ...
    @pytest.mark.asyncio
    async def test_delete_workspace(self, session: AsyncSession, service: WorkspaceService, single_workspace: Workspace, regular_user):
        """Should delete a workspace through the service."""
        context = {"user_id": regular_user.id}
        result = await service.delete(session, obj_id=single_workspace.id, context=context)

        assert result is True

        # Verify deletion
        deleted_workspace = await service.get(session, obj_id=single_workspace.id, context=context)
