...
class TestOutboxToNotificationFlowIntegration:
    ...
    @pytest.fixture
    def memo_service(self, memo_repo: MemoRepository, workspace_repo: WorkspaceRepository) -> MemoService:
    ...
    async def test_memo_creation_triggers_notification_via_outbox(
        ...
        notifications = await notification_repo.get_multi(
            inspect_session, where=[NotificationRepository.model.user_id == regular_user.id]
        )
